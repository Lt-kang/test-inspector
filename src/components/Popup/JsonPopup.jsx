...
export default function JsonViewerPopup() {
  const { fileName, metaData, masterProblemData } = useContext(ProblemDataContext);
  const { isJsonPopupOpen, setIsJsonPopupOpen } = useContext(PopupContext);


  const [mergedData, setMergedData] = useState({file_name: fileName, meta: metaData, data: masterProblemData});
  useEffect(() => {
    setMergedData({file_name: fileName, meta: metaData, data: masterProblemData});
  }, [fileName, metaData, masterProblemData]);
  ...
  const handleCopy = async () => {
    try {
      const textToCopy = formatJsonString(mergedData);
      ...
      textArea.value = formatJsonString(mergedData);
  ...
  if (!isJsonPopupOpen) return null;
